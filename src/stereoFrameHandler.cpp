...
void StereoFrameHandler::removeOutliers(Matrix4d DT)
{
    ...
    for( list<LineFeature*>::iterator it = matched_ls.begin(); it!=matched_ls.end(); it++, iter++)
    {
        ...
        //double overlap = lineSegmentOverlap( (*it)->spl, (*it)->epl, spl_proj, epl_proj );
        //ove_l.push_back(overlap);
    }
    ...
    //double inlier_th_o =  1.0 - Config::inlierK() * vector_stdv_mad_nozero( ove_l );
    ...
    for( list<LineFeature*>::iterator it = matched_ls.begin(); it!=matched_ls.end(); it++, iter++)
    {
        //if( res_l[iter] > inlier_th_l || ove_l[iter] < inlier_th_o )
