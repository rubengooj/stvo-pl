...
MatrixKitti::MatrixKitti () {
  m   = 0;
  n   = 0;
  val = 0;
}

MatrixKitti::MatrixKitti (const int32_t m_,const int32_t n_) {
  allocateMemory(m_,n_);
}

MatrixKitti::MatrixKitti (const int32_t m_,const int32_t n_,const FLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
MatrixKitti::MatrixKitti (const MatrixKitti &M) {
...
MatrixKitti& MatrixKitti::operator= (const MatrixKitti &M) {
  if (this!=&M) {
    if (M.m!=m || M.n!=n) {
      releaseMemory();
      allocateMemory(M.m,M.n);
...
void MatrixKitti::getData(FLOAT* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
MatrixKitti MatrixKitti::getMat(int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i1<0 || i2>=m || j1<0 || j2>=n || i2<i1 || j2<j1) {
    cerr << "ERROR: Cannot get subMatrixKitti [" << i1 << ".." << i2 <<
        "] x [" << j1 << ".." << j2 << "]" <<
        " of a (" << m << "x" << n << ") MatrixKitti." << endl;
    exit(0);
  }
  MatrixKitti M(i2-i1+1,j2-j1+1);
  for (int32_t i=0; i<M.m; i++)
...
void MatrixKitti::setMat(const MatrixKitti &M,const int32_t i1,const int32_t j1) {
  if (i1<0 || j1<0 || i1+M.m>m || j1+M.n>n) {
    cerr << "ERROR: Cannot set subMatrixKitti [" << i1 << ".." << i1+M.m-1 <<
        "] x [" << j1 << ".." << j1+M.n-1 << "]" <<
        " of a (" << m << "x" << n << ") MatrixKitti." << endl;
    exit(0);
  }
...
void MatrixKitti::setVal(FLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void MatrixKitti::setDiag(FLOAT s,int32_t i1,int32_t i2) {
  if (i2==-1) i2 = min(m-1,n-1);
  for (int32_t i=i1; i<=i2; i++)
    val[i][i] = s;
}

void MatrixKitti::zero() {
  setVal(0);
}

MatrixKitti MatrixKitti::extractCols (vector<int> idx) {
  MatrixKitti M(m,idx.size());
  for (int32_t j=0; j<M.n; j++)
    if (idx[j]<n)
...
MatrixKitti MatrixKitti::eye (const int32_t m) {
  MatrixKitti M(m,m);
  for (int32_t i=0; i<m; i++)
    M.val[i][i] = 1;
  return M;
...
MatrixKitti MatrixKitti::diag (const MatrixKitti &M) {
  if (M.m>1 && M.n==1) {
    MatrixKitti D(M.m,M.m);
    for (int32_t i=0; i<M.m; i++)
      D.val[i][i] = M.val[i][0];
    return D;
  } else if (M.m==1 && M.n>1) {
    MatrixKitti D(M.n,M.n);
    for (int32_t i=0; i<M.n; i++)
      D.val[i][i] = M.val[0][i];
    return D;
  }
  cout << "ERROR: Trying to create diagonal MatrixKitti from vector of size (" << M.m << "x" << M.n << ")" << endl;
  exit(0);
}

MatrixKitti MatrixKitti::reshape(const MatrixKitti &M,int32_t m_,int32_t n_) {
  if (M.m*M.n != m_*n_) {
    cerr << "ERROR: Trying to reshape a MatrixKitti of size (" << M.m << "x" << M.n <<
            ") to size (" << m_ << "x" << n_ << ")" << endl;
    exit(0);
  }
  MatrixKitti M2(m_,n_);
...
MatrixKitti MatrixKitti::rotMatX (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  MatrixKitti R(3,3);
  R.val[0][0] = +1;
...
MatrixKitti MatrixKitti::rotMatY (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  MatrixKitti R(3,3);
  R.val[0][0] = +c;
...
MatrixKitti MatrixKitti::rotMatZ (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  MatrixKitti R(3,3);
  R.val[0][0] = +c;
...
MatrixKitti MatrixKitti::operator+ (const MatrixKitti &M) {
  const MatrixKitti &A = *this;
  const MatrixKitti &B = M;
  if (A.m!=B.m || A.n!=B.n) {
    cerr << "ERROR: Trying to add matrices of size (" << A.m << "x" << A.n <<
        ") and (" << B.m << "x" << B.n << ")" << endl;
    exit(0);
  }
  MatrixKitti C(A.m,A.n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
...
MatrixKitti MatrixKitti::operator* (const FLOAT &s) {
  MatrixKitti C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
MatrixKitti MatrixKitti::operator- () {
  MatrixKitti C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = -val[i][j];
...
MatrixKitti MatrixKitti::cross (const MatrixKitti &a, const MatrixKitti &b) {
...
MatrixKitti MatrixKitti::inv (const MatrixKitti &M) {
  if (M.m!=M.n) {
    cerr << "ERROR: Trying to invert MatrixKitti of size (" << M.m << "x" << M.n << ")" << endl;
    exit(0);
  }
  MatrixKitti A(M);
  MatrixKitti B = eye(M.m);
  B.solve(A);
  return B;
}

bool MatrixKitti::inv () {
  if (m!=n) {
    cerr << "ERROR: Trying to invert MatrixKitti of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
  MatrixKitti A(*this);
  eye();
...
FLOAT MatrixKitti::det () {
  
  if (m != n) {
    cerr << "ERROR: Trying to compute determinant of a MatrixKitti of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
...
bool MatrixKitti::solve (const MatrixKitti &M, FLOAT eps) {
  
  // substitutes
  const MatrixKitti &A = M;
  MatrixKitti &B       = *this;
...
// Given a MatrixKitti a[1..n][1..n], this routine replaces it by the LU decomposition of a rowwise
// permutation of itself. a and n are input. a is output, arranged as in equation (2.3.14) above;
// indx[1..n] is an output vector that records the row permutation effected by the partial
// pivoting; d is output as ±1 depending on whether the number of row interchanges was even
// or odd, respectively. This routine is used in combination with lubksb to solve linear equations
// or invert a MatrixKitti.

bool MatrixKitti::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
  
  if (m != n) {
    cerr << "ERROR: Trying to LU decompose a MatrixKitti of size (" << m << "x" << n << ")" << endl;
    exit(0);
  }
...
// Given a MatrixKitti M/A[1..m][1..n], this routine computes its singular value decomposition, M/A =
// U·W·V T. TheMatrixKitti U replaces a on output. The diagonal MatrixKitti of singular values W is output
// as a vector w[1..n]. TheMatrixKitti V (not the transpose V T ) is output as v[1..n][1..n].
void MatrixKitti::svd(MatrixKitti &U2,MatrixKitti &W,MatrixKitti &V) {

  MatrixKitti U = MatrixKitti(*this);
  U2 = MatrixKitti(m,m);
  V  = MatrixKitti(n,n);
  ...
  W = MatrixKitti(min(m,n),1,w);
  
  // extract mxm subMatrixKitti U
...
ostream& operator<< (ostream& out,const MatrixKitti& M) {
  if (M.m==0 || M.n==0) {
    out << "[empty MatrixKitti]";
  } else {
    char buffer[1024];
    for (int32_t i=0; i<M.m; i++) {
      for (int32_t j=0; j<M.n; j++) {
...
void MatrixKitti::allocateMemory (const int32_t m_,const int32_t n_) {
  m = abs(m_); n = abs(n_);
  if (m==0 || n==0) {
    val = 0;
    return;
...
void MatrixKitti::releaseMemory () {
  if (val!=0) {
    free(val[0]);
    free(val);
  }
...
FLOAT MatrixKitti::pythag(FLOAT a,FLOAT b) {
  FLOAT absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
